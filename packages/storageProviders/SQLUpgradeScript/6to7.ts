...
import { DBVersion, MessageInSQLDB, MsgTableName } from '@icalingua/types/SQLTableTypes'

/** 应该是最麻烦、问题最多的一次升级。该次升级旨在取消动态表名，将所有房间的消息存到同一个表内，
 * 从而解决烦人的问题。
 */
const upg6to7 = async (db: Knex, dbType: 'pg' | 'mysql' | 'sqlite3') => {
    console.log('Applying SQL Storage Provider Update:\n', 'Version 7\n', 'Fill the Rabbit Hole')
    const msgTableNames = await db<MsgTableName>('msgTableName').select('tableName')
        ...
        const PAry = msgTableNamesAry.map(async (msgTableName) => {
            ...
            if (!hasMsgTable) {
                ...
                return new Promise<void>((resolve) => {
                    ...
                    const messageStream = db<Message>(msgTableName).select('*').stream()
        ...
        for await (const msgTableName of msgTableNamesAry) {
            ...
            if (hasMsgTable) {
                ...
                for (let i = 0; i <= msgCountNum; i += 200) {
                    try {
                        const messages = await db<Message>(msgTableName).select('*').offset(i).limit(200)
                        await db<MessageInSQLDB>('messages')
                            .insert(
                                messages.map((data) => ({
                                    ...data,
